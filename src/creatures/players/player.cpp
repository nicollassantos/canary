...
std::string Player::getDescription(int32_t lookDistance) {
	return m_combatStatsComponent.getDescription(lookDistance);
}
...
uint16_t Player::getWeaponId(bool ignoreAmmo /* = false */) const {
	return m_combatStatsComponent.getWeaponId(ignoreAmmo);
}

std::shared_ptr<Item> Player::getWeapon(Slots_t slot, bool ignoreAmmo) const {
	return m_combatStatsComponent.getWeapon(slot, ignoreAmmo);
}

bool Player::hasQuiverEquipped() const {
	return m_combatStatsComponent.hasQuiverEquipped();
}

bool Player::hasWeaponDistanceEquipped() const {
	return m_combatStatsComponent.hasWeaponDistanceEquipped();
}

std::shared_ptr<Item> Player::getQuiverAmmoOfType(const ItemType &it) const {
	return m_combatStatsComponent.getQuiverAmmoOfType(it);
}

std::shared_ptr<Item> Player::getWeapon(bool ignoreAmmo /* = false*/) const {
	return m_combatStatsComponent.getWeapon(ignoreAmmo);
}

WeaponType_t Player::getWeaponType() const {
	return m_combatStatsComponent.getWeaponType();
}

int32_t Player::getWeaponSkill(const std::shared_ptr<Item> &item) const {
	return m_combatStatsComponent.getWeaponSkill(item);
}

uint16_t Player::getDistanceAttackSkill(const int32_t attackSkill, const int32_t weaponAttack) const {
	return m_combatStatsComponent.getDistanceAttackSkill(attackSkill, weaponAttack);
}

uint16_t Player::getAttackSkill(const std::shared_ptr<Item> &item) const {
	return m_combatStatsComponent.getAttackSkill(item);
}

uint8_t Player::getWeaponSkillId(const std::shared_ptr<Item> &item) const {
	return m_combatStatsComponent.getWeaponSkillId(item);
}

uint16_t Player::calculateFlatDamageHealing() const {
	return m_combatStatsComponent.calculateFlatDamageHealing();
}

uint16_t Player::attackTotal(uint16_t flatBonus, uint16_t equipment, uint16_t skill) const {
	return m_combatStatsComponent.attackTotal(flatBonus, equipment, skill);
}

uint16_t Player::attackRawTotal(uint16_t flatBonus, uint16_t equipment, uint16_t skill) const {
	return m_combatStatsComponent.attackRawTotal(flatBonus, equipment, skill);
}

int32_t Player::getArmor() const {
	return m_combatStatsComponent.getArmor();
}

int32_t Player::getMantra() const {
	return m_combatStatsComponent.getMantra();
}

int32_t Player::getPartyMantra() const {
	return m_combatStatsComponent.getPartyMantra();
}

void Player::updatePartyMantra() const {
	m_combatStatsComponent.updatePartyMantra();
}

void Player::getShieldAndWeapon(std::shared_ptr<Item> &shield, std::shared_ptr<Item> &weapon) const {
	m_combatStatsComponent.getShieldAndWeapon(shield, weapon);
}

float Player::getMitigation() const {
	return m_combatStatsComponent.getMitigation();
}

double Player::getCombatTacticsMitigation() const {
	return m_combatStatsComponent.getCombatTacticsMitigation();
}

int32_t Player::getDefense(bool sendToClient /* = false*/) const {
	return m_combatStatsComponent.getDefense(sendToClient);
}

uint16_t Player::getDefenseEquipment() const {
	return m_combatStatsComponent.getDefenseEquipment();
}

float Player::getAttackFactor() const {
	return m_combatStatsComponent.getAttackFactor();
}

float Player::getDefenseFactor(bool sendToClient /* = false*/) const {
	return m_combatStatsComponent.getDefenseFactor(sendToClient);
}

std::vector<double> Player::getDamageAccuracy(const ItemType &it) const {
	return m_combatStatsComponent.getDamageAccuracy(it);
}
...
ReturnValue Player::queryAdd(int32_t index, const std::shared_ptr<Thing> &thing, uint32_t count, uint32_t flags, const std::shared_ptr<Creature> &actor) {
	return m_cylinderComponent.queryAdd(index, thing, count, flags, actor);
}
ReturnValue Player::queryMaxCount(int32_t index, const std::shared_ptr<Thing> &thing, uint32_t count, uint32_t &maxQueryCount, uint32_t flags) {
	return m_cylinderComponent.queryMaxCount(index, thing, count, maxQueryCount, flags);
}
ReturnValue Player::queryRemove(const std::shared_ptr<Thing> &thing, uint32_t count, uint32_t flags, const std::shared_ptr<Creature> &actor) {
	return m_cylinderComponent.queryRemove(thing, count, flags, actor);
}
...
void Player::internalAddThing(const std::shared_ptr<Thing> &thing) {
	m_cylinderComponent.internalAddThing(thing);
}

void Player::internalAddThing(uint32_t index, const std::shared_ptr<Thing> &thing) {
	m_cylinderComponent.internalAddThing(index, thing);
}

// safe-trade functions// safe-trade functions
...
void Player::postAddNotification(const std::shared_ptr<Thing> &thing, const std::shared_ptr<Cylinder> &oldParent, int32_t index, CylinderLink_t link) {
	m_cylinderComponent.postAddNotification(thing, oldParent, index, link);
}
void Player::postRemoveNotification(const std::shared_ptr<Thing> &thing, const std::shared_ptr<Cylinder> &newParent, int32_t index, CylinderLink_t link) {
	m_cylinderComponent.postRemoveNotification(thing, newParent, index, link);
}
...
void Player::triggerMomentum() {
	m_forgeComponent.triggerMomentum();
}
...
void Player::triggerTranscendence() {
	m_forgeComponent.triggerTranscendence();
}
...
void Player::forgeFuseItems(ForgeAction_t actionType, uint16_t firstItemId, uint8_t tier, uint16_t secondItemId, bool success, bool reduceTierLoss, bool convergence, uint8_t bonus, uint8_t coreCount) {
	m_forgeComponent.forgeFuseItems(actionType, firstItemId, tier, secondItemId, success, reduceTierLoss, convergence, bonus, coreCount);
}
...
void Player::forgeTransferItemTier(ForgeAction_t actionType, uint16_t donorItemId, uint8_t tier, uint16_t receiveItemId, bool convergence) {
	m_forgeComponent.forgeTransferItemTier(actionType, donorItemId, tier, receiveItemId, convergence);
}

void Player::forgeResourceConversion(ForgeAction_t actionType) {
	m_forgeComponent.forgeResourceConversion(actionType);
}

void Player::forgeHistory(uint8_t page) const {
	m_forgeComponent.forgeHistory(page);
}

void Player::sendOpenForge() const {
	m_forgeComponent.sendOpenForge();
}

void Player::sendForgeError(ReturnValue returnValue) const {
	m_forgeComponent.sendForgeError(returnValue);
}
...
void Player::sendForgeResult(ForgeAction_t actionType, uint16_t leftItemId, uint8_t leftTier, uint16_t rightItemId, uint8_t rightTier, bool success, uint8_t bonus, uint8_t coreCount, bool convergence) const {
	m_forgeComponent.sendForgeResult(actionType, leftItemId, leftTier, rightItemId, rightTier, success, bonus, coreCount, convergence);
}

void Player::sendForgeHistory(uint8_t page) const {
	m_forgeComponent.sendForgeHistory(page);
}

void Player::closeForgeWindow() const {
	m_forgeComponent.closeForgeWindow();
}

void Player::setForgeDusts(uint64_t amount) {
	m_forgeComponent.setForgeDusts(amount);
}

void Player::addForgeDusts(uint64_t amount) {
	m_forgeComponent.addForgeDusts(amount);
}

void Player::removeForgeDusts(uint64_t amount) {
	m_forgeComponent.removeForgeDusts(amount);
}

uint64_t Player::getForgeDusts() const {
	return m_forgeComponent.getForgeDusts();
}

void Player::addForgeDustLevel(uint64_t amount) {
	m_forgeComponent.addForgeDustLevel(amount);
}

void Player::removeForgeDustLevel(uint64_t amount) {
	m_forgeComponent.removeForgeDustLevel(amount);
}

uint64_t Player::getForgeDustLevel() const {
	return m_forgeComponent.getForgeDustLevel();
}

void Player::registerForgeHistoryDescription(ForgeHistory history) {
	m_forgeComponent.registerForgeHistoryDescription(std::move(history));
}
...
SoundEffect_t Player::getAttackSoundEffect() const {
	return m_combatStatsComponent.getAttackSoundEffect();
}

SoundEffect_t Player::getHitSoundEffect() const {
	return m_combatStatsComponent.getHitSoundEffect();
}
...
PlayerCylinderComponent &Player::cylinderComponent() {
	return m_cylinderComponent;
}

const PlayerCylinderComponent &Player::cylinderComponent() const {
	return m_cylinderComponent;
}
...
PlayerForgeComponent &Player::forgeComponent() {
